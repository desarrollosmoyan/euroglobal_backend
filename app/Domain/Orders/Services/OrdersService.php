...
use Domain\Orders\DataTransferObjects\OrderSummarizedProductionReportPDFResponse;
use Domain\Orders\DataTransferObjects\OrderSummarizedProductionSearchRequest;
use Domain\Orders\DataTransferObjects\OrderTicketPDFResponse;
use Domain\Orders\Exports\ProductionReportExport;
use Domain\Orders\Exports\SummarizedProductionReportExport;
...
class OrdersService implements \Domain\Orders\Contracts\Services\OrdersService
{
     ...
     * @return string|null
     */
    public function productionReportExcel(OrderDetailSearchRequest $request): ?string
    {
        $request->includes = ['product', 'product.productType', 'order', 'order.client', 'order.company'];
        $request->paginateSize = config('system.infinite_pagination');
        ...
        $dates = [
            'from' => array_key_exists('created_at_from', $request->filters) && !empty($request->filters['created_at_from']) ? $request->filters['created_at_from'] : null,
            'to' => array_key_exists('created_at_to', $request->filters) && !empty($request->filters['created_at_to']) ? $request->filters['created_at_to'] : null,
        ];

        $excel = new ProductionReportExport($records,$dates);
        $name = uniqid() . '.xlsx';
    ...
    /**
     * @param OrderSummarizedProductionSearchRequest $request
     * @return OrderSummarizedProductionReportPDFResponse
     */
    public function summarizedProductionReport(OrderSummarizedProductionSearchRequest $request): OrderSummarizedProductionReportPDFResponse
    {
        if (array_key_exists('product_type_id', $request->filters) && !empty($request->filters['product_type_id'])) {
            if (is_string($request->filters['product_type_id'])) {
                $productTypeIds = explode(',', $request->filters['product_type_id']);
            ...
            $request->filters['product_type_id'] = $productTypeIds;
        }

        $query = $this->orderDetailsRepository->summarized(
            $request->filters,
            $request->sortField,
            $request->sortType
        );
        ...
        $pdf = Pdf::loadView('pdf.summarized_production_report', [
            'data' => [
                'records' => $records
            ],
            'filters' => $request->filters,
        ...
        return (new OrderSummarizedProductionReportPDFResponse(Response::STATUSES['OK']))
            ->setSuccess()
            ->setData(
                [
                    'title' => 'Resumen Informe Producción',
                    'content' => base64_encode($pdf->output())
                ]
            );
    }
     ...
     * @return string|null
     */
    public function summarizedProductionReportExcel(OrderSummarizedProductionSearchRequest $request): ?string
    {
        if (array_key_exists('product_type_id', $request->filters) && !empty($request->filters['product_type_id'])) {
            if (is_string($request->filters['product_type_id'])) {
                $productTypeIds = explode(',', $request->filters['product_type_id']);
        ...
        $excel = new SummarizedProductionReportExport($records,$dates);
        $name = uniqid() . '.xlsx';
        app(Excel::class)->store($excel, $name);
