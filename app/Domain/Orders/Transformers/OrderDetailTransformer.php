...
use Support\Transformers\SaltRoomReservationsServiceSaltRoomReservationTransformer;
...
class OrderDetailTransformer extends Transformer
{
    ...
    public function transform(OrderDetail $entity): array
    {
        return [
            ...
            'salt_sessions' => $entity->salt_sessions,
     ...
     * @throws UnknownProperties
     */
    public function includeSaltRoomReservations(OrderDetail $entity): ?Collection
    {
        $reservations = app(SaltRoomReservationsService::class)->findByOrderDetail($entity->id);

        return $reservations->getData()->count() ? $this->collection(
            $reservations->getData()->pluck('id')->values()->toArray(),
            app(SaltRoomReservationsServiceSaltRoomReservationTransformer::class)
        ) : null;
    }
}
