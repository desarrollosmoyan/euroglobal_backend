...
class CreateOrderDetail
{
    ...
    public function __construct(
        ...
        private readonly SaltRoomReservationsService $saltRoomReservationsService,
    ...
    public function __invoke(array $data): OrderDetail
    {
        ...
        if (!empty($data['reservations'])) {
            ...
            foreach ($data['reservations'] as $reservation) {
                ...
                if (array_key_exists('ignore_festive_validation', $reservation)) {
                    $reservationData['ignore_festive_validation'] = $reservation['ignore_festive_validation'];
                }
                ...
                } elseif ($reservation['type'] === 'SaltRoom') {
                    $this->saltRoomReservationsService->$reservationAction([
                        'adults' => $reservation['adults'],
                        'children' => $reservation['children'],
                        ...$reservationData
                    ]);
    ...
    private function rules(): array
    {
        return [
            ...
            'salt_sessions' => 'nullable|numeric',
            'reservations' => 'nullable|array',
            'reservations.*.type' => 'required|in:Circuit,Treatment,SaltRoom',
            ...
            'reservations.*.date' => Rule::forEach(function ($value, $attribute, $data) {
                ...
                if (!array_key_exists(str_replace('.date', '.ignore_festive_validation', $attribute), $data)) {
                    $time = $data[str_replace('.date', '.time', $attribute)];
                    $rules[] = function ($attribute, $value, $fail) use ($festiveRecords, $time) {
            ...
            'reservations.*.adults' => 'nullable|required_if:reservations.*.type,Circuit,SaltRoom|numeric',
            'reservations.*.children' => 'nullable|required_if:reservations.*.type,Circuit,SaltRoom|numeric',
