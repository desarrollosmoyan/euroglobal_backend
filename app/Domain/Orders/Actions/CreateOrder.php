...
class CreateOrder
{
    ...
    private function rules($data): array
    {
        return [
            ...
            'details.*.salt_sessions' => 'nullable|numeric',
            'details.*.reservations' => 'nullable|array',
            'details.*.reservations.*.type' => 'required|in:Circuit,Treatment,SaltRoom',
            ...
            'details.*.reservations.*.adults' => 'nullable|required_if:details.*.reservations.*.type,Circuit,SaltRoom|numeric',
            'details.*.reservations.*.children' => 'nullable|required_if:details.*.reservations.*.type,Circuit,SaltRoom|numeric',
            'details.*.reservations.*.employee_id' => 'nullable|numeric',
            'counter_sale_seq' => 'nullable|max:255|unique:orders,counter_sale_seq',
