...
class ClientsRepository extends Repository implements RepositoryInterface
{
    ...
    public function searchQueryBuilder(array $filters, string $sortField, SQLSort $sortType): Builder
    {
        ...
        if (array_key_exists('partner_number', $filters)) {
            $query->where('partner_number', '=', $filters['partner_number']);
        }

        if (array_key_exists('is_partner', $filters)) {
            if ($filters['is_partner']) {
                $query->whereNotNull('partner_number');
            } else {
