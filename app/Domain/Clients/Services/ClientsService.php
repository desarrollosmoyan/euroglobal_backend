...
use Domain\Clients\DataTransferObjects\ClientMakePartnerResponse;
...
class ClientsService implements \Domain\Clients\Contracts\Services\ClientsService
{
    ...
    {
        $records = $this->repository->searchQueryBuilder(
            $request->filters,
            $request->sortField,
            $request->sortType
        );

        return app(ClientsExport::class)($records);
    }
    ...
    /**
     * @param integer $id
     * @return ClientMakePartnerResponse
     */
    public function makePartner(int $id): ClientMakePartnerResponse
    {
        $response = app(ClientMakePartnerResponse::class, ['status' => 'Ok']);

        $client = $this->find($id);

        if (!$client) {
            return $response->setErrors(['The resource you are looking for does not exist.']);
        }
        if ($client->partner_number) {
            return $response->setErrors(['The client has already been registered as a partner.']);
        }

        $lastClient = Client::orderBy('partner_number', 'desc')->first();
        $partnerNumber = $lastClient ? $lastClient->partner_number + 1 : 1;

        $client->partner_number = $partnerNumber;

        $result = $this->update($client->toArray());
