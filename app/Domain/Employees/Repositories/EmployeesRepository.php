...
class EmployeesRepository extends Repository implements RepositoryInterface
{
    ...
    {
        $query = $this->getEntity()->newQuery()->select('employees.*');
        ...
        if (array_key_exists('working_on_day', $filters) && !empty($filters['working_on_day'])) {
            $query->join('employee_working_hours', 'employees.id', 'employee_working_hours.employee_id')
                ->where('employee_working_hours.date', '=', $filters['working_on_day']);
        }
