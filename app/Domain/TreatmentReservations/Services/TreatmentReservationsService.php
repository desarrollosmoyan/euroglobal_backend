    ...
    \Domain\TreatmentReservations\Contracts\Services\TreatmentReservationsService
{
    ...
    public function schedulesPdf(string $date, Request $request): TreatmentReservationSchedulesPdfResponse
    {
        ...
        $employeesWorkingHours = $this->employeesService->searchEmployeeWorkingHours(
        ...
        $employeesData = $employees->getData()
            ->filter(function ($employee) use ($records, $employeesTimeOff, $employeesWorkingHours) {
                // Si el empleado tiene algun tratamiento agendado lo incluimos incluso si no le toca trabajar ese dia.
                if ($records->getData()->where('employee_id', $employee->id)->count()) {
                    return true;
                }
                // si esta de vacaciones lo excluimos
                if ($employeesTimeOff->getData()->where('employee_id', $employee->id)->count()) {
                    return false;
                }
                // Si no tiene horas de trabajo ese dia lo excluimos
                if (!$employeesWorkingHours->getData()->where('employee_id', $employee->id)->count()) {
                    return false;
                }
                return true;
            });
        ...
        $pdf = Pdf::loadView('pdf.treatment_reservations_schedules', [
            'data' => [
                ...
                'employeesWorkingHours' => $employeesWorkingHours,
                'currentDate' => $date,
    ...
    public function schedulesEmployeePdf(string $date, Employee $employee, Request $request): TreatmentReservationSchedulesPdfResponse
    {
        $searchRequest = app(TreatmentReservationSearchRequest::class, [
            'args' => [
                'filters' => ['date' => $date, 'employee_id' => $employee->id],
                'includes' => ['client', 'orderDetails.product', 'orderDetails.order'],
                'paginateSize' => config('system.infinite_pagination'),
                'sortField' => 'time',
                'sortType' => SQLSort::SORT_ASC
            ]
        ...
        $employeeWorkingHours = $this->employeesService->searchEmployeeWorkingHours(
            new EmployeeWorkingHoursSearchRequest([
                'filters' => [
                    'date' => $date,
                    'employee_id' => $employee->id,
