...
class UpsertProduct
{
    ...
    private function rules($data): array
    {
        $rules = [
            ...
            'salt_sessions' => 'required|numeric',
            ...
            'duration_salt_schedule' => Rule::requiredIf(fn () => (array_key_exists('salt_sessions', $data) && $data['salt_sessions'] > 0)),
    ...
    private function orderByProductPriority($data): void
    {
        ...
        if (isset($data['id'])) {
            ...
            } elseif ($priority > $product->priority) {
                $currentPriorities = Product::where('priority', '>', $product->priority)
                    ->where('priority', '<=', $priority)
                    ->where('product_type_id', $product_type_id)
                    ->orderBy('priority')
