...
class OrdersController extends Controller
{
     ...
     * @throws UnknownProperties
     */
    public function productionReportExcel(Request $request): JsonResponse
    {
        set_time_limit(0);
        ini_set('memory_limit', '2000M');

        $result = $this->ordersService->productionReportExcel(
            new OrderDetailSearchRequest([
                'filters' => $request->except(['includes', 'paginate_size'])
            ])
        );
        ...
        return $this->apiOkResponse(['file' => $result, 'content' => base64_encode($content)]);
    }
     ...
     * @throws UnknownProperties
     */
    public function summarizedProductionReport(Request $request): JsonResponse
    {
        $pdf = $this->ordersService->summarizedProductionReport(
            new OrderSummarizedProductionSearchRequest([
                'filters' => $request->except(['includes', 'paginate_size'])
            ])
        );
     ...
     * @throws UnknownProperties
     */
    public function summarizedProductionReportExcel(Request $request): JsonResponse
    {
        set_time_limit(0);
        ini_set('memory_limit', '2000M');

        $result = $this->ordersService->summarizedProductionReportExcel(
            new OrderSummarizedProductionSearchRequest([
                'filters' => $request->except(['includes', 'paginate_size'])
            ])
        );
