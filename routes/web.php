...
Route::post('/website/create-order', function () {
    ...
    if (array_key_exists('details', $data)) {
        foreach ($data['details'] as $detail) {
            $discount = !empty($detail['discount']) ? $detail['discount'] / $detail['qty'] : 0;
            if (empty($detail['date'])) {
                $purchases[] = $detail;
                $purchasesTotalPrice += $detail['qty'] * ($detail['price'] - $discount);
            } else {
                $reservations[] = $detail;
                $reservationsTotalPrice += $detail['qty'] * ($detail['price'] - $discount);
            }
        }
    } else {
